    ...
    try {
      ...
      if (url.pathname === "/clean") {
        ...
        if (password !== ADMIN_PASSWORD && !(await checkTempPassword(password))) {
          return new Response(JSON.stringify({ error: "无权限" }), { status: 403, headers: AllowedOrigins });
        }
        await cleanExpiredKeys();
        return new Response(JSON.stringify({ message: "清理任务已触发" }), { status: 200, headers: AllowedOrigins });
      }
      ...
      if (url.pathname === "/me") {
        ...
        if (!name) return new Response(JSON.stringify({ error: "未登录" }), { status: 403, headers: AllowedOrigins });
        return new Response(JSON.stringify({ name }), { status: 200, headers: AllowedOrigins });
      }
      ...
      if (url.pathname === "/register") {
        ...
        if (usernameErr) return new Response(JSON.stringify({ error: usernameErr }), { status: 400, headers: AllowedOrigins });

        const passwordErr = validatePassword(password);
        if (passwordErr) return new Response(JSON.stringify({ error: passwordErr }), { status: 400, headers: AllowedOrigins });

        const keyRes = await kv.get(["invitekey", key]);
        if (!keyRes.value) return new Response(JSON.stringify({ error: "邀请码无效" }), { status: 400, headers: AllowedOrigins });
        if (keyRes.value < Date.now()) {
          await kv.delete(["invitekey", key]);
          return new Response(JSON.stringify({ error: "邀请码已过期" }), { status: 400, headers: AllowedOrigins });
        }

        const user = await kv.get(["user", name]);
        if (user.value) return new Response(JSON.stringify({ error: "用户名已存在" }), { status: 400, headers: AllowedOrigins });
        await kv.set(["user", name], argon2(password));
        await kv.delete(["invitekey", key]);

        return new Response(JSON.stringify({ message: "注册成功" }), { status: 200, headers: AllowedOrigins });
      }
      ...
      if (url.pathname === "/newInviteKey") {
        const { password } = data;
        if (password !== ADMIN_PASSWORD && !(await checkTempPassword(password))) {
          return new Response(JSON.stringify({ error: "密码无效" }), { status: 403, headers: AllowedOrigins });
        }
        const key = Math.random().toString(36).slice(-8);
        const validityTime = Date.now() + 7 * 24 * 60 * 60 * 1000;
        await kv.set(["invitekey", key], validityTime);
        return new Response(JSON.stringify({ key, validUntil: validityTime }), { status: 200, headers: AllowedOrigins });
      }
      ...
      if (url.pathname === "/addTempPassword") {
        ...
        if (password !== ADMIN_PASSWORD) {
          return new Response(JSON.stringify({ error: "只有站长可添加临时密码" }), { status: 403, headers: AllowedOrigins });
        }
        const pwErr = validatePassword(newPassword, 10);
        if (pwErr) return new Response(JSON.stringify({ error: pwErr }), { status: 400, headers: AllowedOrigins });

        const temp = await kv.get(["tempPassword", newPassword]);
        if (temp.value) return new Response(JSON.stringify({ error: "临时密码已存在" }), { status: 400, headers: AllowedOrigins });

        const validityTime = Date.now() + 7 * 24 * 60 * 60 * 1000;
        await kv.set(["tempPassword", newPassword], validityTime);

        return new Response(JSON.stringify({ message: "临时密码添加成功", validUntil: validityTime }), { status: 200, headers: AllowedOrigins });
      }
      ...
      if (url.pathname === "/login") {
        ...
        const user = await kv.get(["user", name]);
        if (!user.value) return new Response(JSON.stringify({ error: "用户名或密码错误" }), { status: 400, headers: AllowedOrigins });

        // 迁移旧明文密码
        if (user.value.password) {
          const hashedUser = argon2(user.value.password);
          await kv.set(["user", name], hashedUser);
          return new Response(JSON.stringify({ error: "数据刚刚迁移完成，请重试。" }), { status: 400, headers: AllowedOrigins });
        }
        ...
        if (user.value.hashed !== hashedAttempt) {
          return new Response(JSON.stringify({ error: "用户名或密码错误" }), { status: 400, headers: AllowedOrigins });
        }

        const token = crypto.randomUUID();
        const validityTime = Date.now() + 24 * 60 * 60 * 1000;
        await kv.set(["token", token], { name, validityTime });
        return new Response(JSON.stringify({ token, validUntil: validityTime }), { status: 200, headers: AllowedOrigins });
      }
      ...
      if (url.pathname === "/postComment") {
        ...
        if (!content || content.length === 0) return new Response(JSON.stringify({ error: "评论不能为空" }), { status: 400, headers: AllowedOrigins });
        if (content.length > 500) return new Response(JSON.stringify({ error: "评论过长，限制500字" }), { status: 400, headers: AllowedOrigins });
        ...
        return new Response(JSON.stringify({ message: "评论成功" }), { status: 200, headers: AllowedOrigins });
      }
      ...
      if (url.pathname === "/getComments") {
        ...
        return new Response(
          ...
          { status: 200, headers: AllowedOrigins }
        );
      }

      return new Response(JSON.stringify({ error: "404 Not Found" }), { status: 404, headers: AllowedOrigins });
    } catch (e) {
      console.error(e);
      return new Response(JSON.stringify({ error: "出错了" }), { status: 500, headers: AllowedOrigins });
    }
  },
};
